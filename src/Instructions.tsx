...
First you need to register as an organiser on the website. Click on the log in button and add a username and password. This year we are using a new website system, so log-in details from previous years no longer work. Make sure you remember your chosen password as you will need to create a new account if you forget it. 
...
Click on the PROGRAMME tab and then click on CREATE EVENT at the bottom.
Fill in the different fields. Upload a fitting image. 

We need you to use a valid email address or phone number so that we can contact you ahead of your event. Contact details aren’t publically visible, so if you want them to be, then also include them in the event description.

Select a venue from the listed venues on the website. Pick an available time slot. If you need more time, select a few time slots in a row. If your event start/end time is different from (but within!) the chosen timeslots then include the actual timing in the event description (e.g. 16:15 start).
...
The Antiuni platform doesn’t include a way for people to sign up to attend events, as all events are publically visible. Attendance can vary a lot so be prepared for less/more people than you anticipated. We would encourage you to set up an eventbrite page for your event, and link it in your description. This enables you to require attendees to register beforehand to limit numbers. Alternatively, you could include your email address and ask potential attendees to contact you ahead of time.

Click create! You can save your event as a draft if it is not ready to be publically visible yet. 
...
This is a collaborative effort and we expect organisers to support each other – mention other events when promoting yours, retweet and share.
...
Make sure your space is ready in advance. If it is virtual, do a test run and check that your links are working. If it is physical, make sure it is well signposted and the address is precise.
