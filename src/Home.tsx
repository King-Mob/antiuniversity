...
function Event({ event, venues }: { event: event; venues: venue[] }) {
  ...
  const time = breakIntoDays(event.slotsUsed)
    .map((day) => formatTime(day))
    .join(", ");
...
function Home({
  ...
  const days: day[] = [
    ...
    {
      name: "Wednesday 15th October",
      date: new Date(1760482800000),
      events: [],
    },
    {
      name: "Thursday 16th October",
      date: new Date(1760569200000),
      events: [],
    },
    { name: "Friday 17th October", date: new Date(1760655600000), events: [] },
    {
      name: "Saturday 18th October",
      date: new Date(1760742000000),
      events: [],
    },
    ...
    <>
      {VITE_SUBMISSIONS_OPEN === "true" && (
        <div className="backed-text-container">
          <p className="indent">
            Event submissions open: 20th July - 31st August
          </p>
          <p className="indent">
            View the schedule of events below, and go to{" "}
            <Link to="/instructions">Instructions</Link> to find out how to
        ...
        {days.map((day, dayIndex) =>
          dayIndex === dayActive ? (
            <>
              ...
              {day.events.length === 0 && (
                <p className="backed-text">No events on this day</p>
              )}
            </>
          ) : (
            <>
